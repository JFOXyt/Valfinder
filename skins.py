...
def butaoskins(root,img,result_labels,gray,static_labels):
    ...
    def butaosearch():
        ...
        if url.status_code==200 and entrygets!='':
            ...
            for guntype in range(guntypes):
                nomesarmas=response['data'][guntype]['displayName'].lower().strip()
            ...
            if idskin==404:
                messagebox.showerror('ERROR',"Check if you are conected to the wifi or if you entered a gun skin")
            ...
            if pode:
                for variantes in range(variantes_qnt):
                    ...
                    nome_armat=tkinter.Label(root,text=nomesarmas.capitalize()
                                            ,font=('Arial',44,'bold')
                                            ,bg=gray)
